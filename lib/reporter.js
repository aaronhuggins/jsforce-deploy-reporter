...
class JSforceReporter {
  ...
  async report () {
    ...
    if (junit >= 0 || junitonly >= 0) {
      if (junit >= 0) {
        this.options.reporters.splice(junit, 1)
      } else {
        this.options.reporters.splice(junitonly, 1)
      }
    ...
    if (typeof this.options.junitLevel === 'string') {
      const apexTestResult = await (new ApexTestResult(this.details, this.options)).generate()
