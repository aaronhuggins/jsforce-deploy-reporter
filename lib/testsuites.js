...
class ApexTestResult {
  ...
  async generate () {
    ...
    if (
      ...
      this.components.testsuites.completed = new Date(this.components.testsuites.completed.getTime() - this.tests.testsuites.time)
  ...
  async generateFromTestCases(...testCases) {
    ...
    Object.keys(groupByClassname).forEach((key) => {
      promises.push((async function generateTestSuite (testCases = []) {
        ...
        __this.addTestSuite(new ApexTestSuite({
          id: key,
          name: key,
          failures,
          time,
