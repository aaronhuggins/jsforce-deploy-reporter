...
class ApexTestResult {
  ...
  jeststare () {
    ...
    fs.writeFileSync('./coverage/tests2.json', JSON.stringify(mergedTestSuites.toJest(), null, 2))
...
class ApexTestSuites {
  ...
  toJest() {
    const firstSuite = (this.testSuites[0] || { timestamp: new Date().toISOString() })
    ...
    return {
      ...
      success: (this.failures > 0 ? false : true),
