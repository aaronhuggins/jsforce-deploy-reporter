...
const getApexFileCoverage = async function getApexFileCoverage (
  coverage = {},
  opts = {}
) {
  let executableLines = []
  ...
  const locationsHit =
    parseFloat(coverage.numLocations) -
    parseFloat(coverage.numLocationsNotCovered)
  if (opts.detectExecutableLines) {
    executableLines = detectExecutableLines(sourceFile)
  }
  const lines = parseLines(
    coverage.locationsNotCovered,
    executableLines,
    parseFloat(locationsHit)
  )
...
const parseLines = function parseLines (
  linesMissed = [],
  executableLines = [],
  linesHit
) {
  ...
  for (let i = 0; i < linesHit; i++) {
    ...
    if (!missed.includes(lineNumber) && !hit.includes(lineNumber)) {
      result.push({
        int: lineNumber,
        hit: 1,
        len: null
...
const getTypeFolder = function getTypeFolder (coverageType) {
  ...
  return typeof typeFolders[coverageType] === 'string'
    ? typeFolders[coverageType]
    : ''
}
...
const getTypeExt = function getTypeExt (coverageType) {
  ...
  return typeof typeExts[coverageType] === 'string'
    ? typeExts[coverageType]
    : ''
}
